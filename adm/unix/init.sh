...
# java environment - use JAVA_HOME from environment if set, otherwise hard code>
java_home=$JAVA_HOME
[ -z "$java_home" ] && java_home=/usr/lib/jvm/java-17-openjdk-armhf
export java_home
...
function scodegen
{
  ...
  javac $rt $sedona_home/adm/SCodeGen.java
  $java_home/bin/java -cp $sedona_home/adm SCodeGen "java" $sedona_home/adm/sco>
  $java_home/bin/java -cp $sedona_home/adm SCodeGen "h" $sedona_home/adm/scode.>
